...
<html>
  <head>
	...
	<script type="text/javascript">
		function prestockdrugadd(){
			document.stockdrugForm.action = "${pageContext.request.contextPath }/user/prestockdrugadd.do";
			document.stockdrugForm.submit();
		}
  ...
  <div id="exitbox"><a href="${pageContext.request.contextPath }/user/exitAll.do">退出</a>
  	...
  	<dir id="midbox" style="background-image: url(pic/mid.jpg)">
  		<form name="stockdrugForm" method="post" action="${pageContext.request.contextPath }/user/querystockdrug.do">
  			<table width="100%" border="1">
  				<tr>
  					<td>	
						单据号<input type="text" name="bill_id">
						药品编码<input type="text" name="drug_id">
						采购价<input type="text" name="price">
						<br>
						采购数量<input type="text" name="buy_num">
						采购日期<input type="text" name="date">
		...
		<table width="100%"border=1>
			...
			<c:forEach items="${stokdrugList }" var="stokdrug">
				<tr>
					<td>${stokdrug.bill_id}</td>
					<td>${stokdrug.drug_id}</td>
					<td>${stokdrug.price}</td>
					<td>${stokdrug.buy_num}</td>
					<td>${stokdrug.date}</td>
					<td>${stokdrug.supplier}</td>
					
					<td>
					<a href="${pageContext.request.contextPath }/user/prestockdrugupdate.do?id=${stokdrug.id}">修改</a>
			<!--	<a href="${pageContext.request.contextPath }/user/stockdrugdelete.do?id=${stokdrug.id}">删除</a> -->
					</td>
				</tr>
			</c:forEach>
